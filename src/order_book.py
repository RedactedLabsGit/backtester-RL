...
class OrderBook:
    ...
    def __init__(self, bids : SortedList = SortedList(),
                 asks : SortedList = SortedList()) -> None:
        self.bids = bids
        self.asks = asks
    ...
    def __str__(self):
        ...
        orders = sorted(self.asks + self.bids, key=lambda x: x.price,
                        reverse = True)
    ...
    def to_pandas(self):
        ...
        # Initialize table data
        return pd.DataFrame([(o.order_type, o.qty, o.price, o.qty * o.price) for o in orders],
                            columns = ["Side", "Size", "Price", "Total"]).sort_values(['Side', 'Price'],
...
def add_limit_order(order : Order, 
                order_book : OrderBook) -> OrderBook:
    
    bids = copy.deepcopy(order_book.bids) if (order_book and order_book.bids) else SortedList()
    asks = copy.deepcopy(order_book.asks) if (order_book and order_book.asks) else SortedList()
    ...
    if order.order_type == 'bid':
        try:
            idx_bid = next((i for i, o in enumerate(bids)\
                            if order.price == o.price), None)
        ...
        try:
            idx_ask = next((i for i, o in enumerate(asks)\
                            if order.price == o.price), None)
        except StopIteration:
            if bids:
...
def execute_market_order(price : float,
                        qty : float, 
...
def arbitrage_order_book(price : float,
                         order_book : OrderBook) -> Tuple[List[Order], OrderBook]:
...
def build_book(capital : float, 
    ...
    if initial_price <= price_grid[0]:
        ...
        list_quantity_A = [initial_capital_A / (nb_price_points - 1)] * (nb_price_points - 1)
        asks = SortedList([Order('ask', q, p) for p, q in zip(price_grid[1:], list_quantity_A)])
    ...
    if initial_price >= price_grid[-1]:
        ...
        list_quantity_B = np.array([initial_capital_B / (nb_price_points - 1)] * (nb_price_points - 1)) / np.array(
            sliced_price_grid)
        bids = SortedList([Order('bid', q, p) for p, q in zip(sliced_price_grid, list(list_quantity_B))])
    ...
    for i in range(floor_price0_index - 1, -1, -1):
        ...
        order_book = add_limit_order(Order('bid', list_quantity_B, price_grid[i]),
    ...
    for i in range(floor_price0_index + 1, nb_price_points):
        ...
        order_book = add_limit_order(Order('ask', list_quantity_A, price_grid[i]),
                                     order_book = order_book)
