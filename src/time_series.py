...
class TS:
    ...
    def to_pandas(self):
        return pd.DataFrame(self.values.transpose(), index = self.row_names, columns = self.col_names)
    ...
    def __getitem__(self, index: Union[int, slice, str]):
        if isinstance(index, int):
            if index == 0:
                return TS(row_names=self.row_names[:1],
                          unit=self.unit,
                          n_rows=1,
            ...
            return TS(row_names=self.row_names[start:stop:step],
                      unit=self.unit,
                      n_rows=stop - start,
                      col_names=self.col_names,
                      values=temp_values)
            ...
            return TS(row_names=self.row_names,
                      unit = self.unit,
                      n_rows = self.n_rows,
                      col_names = [index],
...
def get_timedelta_unit(timedelta : pd.Timedelta) -> str:
    if timedelta.components.days != 0:
        ...
        return (timedelta.components.milliseconds, 'ms')  # Default unit if all others are 0
...
def load_csv(path : str, ffill : bool = True) -> TS:
    ...
    if not temp.index.diff().dropna().nunique() == 1:
        if ffill:
            ...
            new_index = pd.date_range(start = temp.index.min(),
                                      end = temp.index.max(),
        ...
        #re-create index
    return TS(row_names = temp.index,
              unit = unit,
...
def col_concat(t : TS, s : TS) -> TS:
    if t.n_rows != s.n_rows:
        ...
        return TS(row_names=t.row_names,
                  unit = t.unit,
                  n_rows=t.n_rows,
