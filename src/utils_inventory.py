...
def concentrator(price, lower_price_bound, upper_price_bound):
    ...
    inverse_concentrator = (
        2 * math.sqrt(price)
        - price / math.sqrt(upper_price_bound)
        - math.sqrt(lower_price_bound)
    )
...
def initial_inventory_allocation(curr_price, p_min, p_max, capital):
    ...
    if curr_price >= p_min and curr_price <= p_max:
        ...
        base_quantity = concentrator_val * (
            1 / math.sqrt(curr_price) - 1 / math.sqrt(p_max)
        )
